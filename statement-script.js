document.addEventListener('DOMContentLoaded', () => {
    ...
    const statementContentArea = document.getElementById('statement-content-area');
    ...
    function getUrlParameters() {
        ...
        if (isNaN(statementMonth) || statementMonth < 0 || statementMonth > 11 || isNaN(statementYear) || statementYear < 2000 || statementYear > 2100) {
    ...
    const displayStatement = () => {
        ...
        if (statementMonth === -1 || statementYear === -1) {
            ...
            statementContentArea.innerHTML = '<p style="color: var(--danger-color);">Could not generate statement due to invalid parameters.</p>';
            return;
        }
        ...
        // Filter Transactions (Logic copied from original generateMonthlyStatement)
         const monthTransactions = transactions.filter(t => {
             try {
                ...
                return !isNaN(transactionDate.getTime()) && transactionDate.getMonth() === statementMonth && transactionDate.getFullYear() === statementYear;
        ...
        if (monthTransactions.length === 0) {
            ...
            monthTransactions.forEach(t => {
                ...
                const amountPrefix = (t.type === 'income' || t.category === 'Savings Deposit') ? '+' : '-'; // Corrected prefix logic slightly
